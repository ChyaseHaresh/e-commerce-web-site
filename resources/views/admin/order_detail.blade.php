    ...
    <div class="row m-t-30 whitebg">
        <div class="col-md-6">
            <div class="order_detail">
                ...
                {{ $orders_details[0]->name }}({{ $orders_details[0]->phone }})
                <br />{{ $orders_details[0]->address }}<br />{{ $orders_details[0]->city }}
            </br>{{ $orders_details[0]->state }}</br />{{ $orders_details[0]->zip }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="order_detail">
                        ...
                        <table class="table order_detail">
                            ...
                            <tbody>
                                ...
                                @foreach ($orders_details as $list)
                                    ...
                                    @endphp
                                    <tr>
                                        <td class="font-weight-bold">{{ $list->pname }}</td>
                                        <td><img width="300" src='{{ asset('storage/media/product/' . $list->attr_image) }}' /></td>
                                        <td>{{ $list->size }}</td>
                                        <td>{{ $list->color }}</td>
                                        <td>{{ $list->price }}</td>
                                        <td>{{ $list->qty }}</td>
